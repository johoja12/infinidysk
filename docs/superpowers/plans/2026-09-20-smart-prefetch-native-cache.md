...
**Architecture:** Add a verified file-range store below the existing shared-stream
pump and above the existing format/NNTP stack. A bounded queue accepts manual and
optional prediction intents; existing provider admission, discovery, repair,
...
- [ ] Implement the manifest protocol from the design, with immutable extents,
  checksums, version/geometry, bounded metadata and exact union coverage.
...
- [ ] Implement bounded catalog recovery, owner lock and revision validation.
  Inject failure before/after each rename/manifest publication and restart.
...
- [ ] Add private file permissions, root ownership marker, symlink/path containment
  checks, and distinct cache roots. Reject a second process claiming the root.
...
| `[0,16)` temp file, no manifest publication | No hit after restart |
...
`Api/NativeCacheReadContractTests.cs`; extend `SegmentCacheNntpClientTests.cs`.
...
- [ ] Add per-purpose native bypass and article-admission suppression. Native-owned
  reads retain article hits and repaired-article precedence; no automatic global
  setting change or old-cache purge. Unsupported fidelity retains normal behavior.
...
- [ ] Test shared on/off, segment on/off, mixed hit/miss batches, HEAD/416, `/view`
  and DAV GET, disconnect, end-of-file, seek behind/ahead, and dual readers.
- [ ] Assert fully cached bytes yield zero BODY calls; a cold eligible read has
  one native publication and no new article-cache publication. Verify bypass
...
- [ ] Expose native hit/miss/committed/invalidated/evicted bytes, writer reservations,
  orphan recovery, duplicate fetches, format failures and metadata counts. Keep
...
- [ ] Add exactly the initial settings/defaults from the design with server-side
...
- [ ] Keep new features advanced/off; add overlap warnings to Settings and wizard
  Review. Test completion allowlists, server strategy bundles, environment-pinned
...
## Task 8 — Integrate verified playback and next-episode prediction

**Create under `backend/Services/Prefetch/`:** `PlaybackSignal.cs`,
`PlaybackSignalRegistry.cs`, `ImportedItemResolver.cs`,
`NextEpisodePrefetchPolicy.cs`, `PlexPlaybackSource.cs`, `EmbyPlaybackSource.cs`.
**Modify:** ConfigKeys/config validation, existing Settings UI and secret resolver
for opt-in adapter settings; register adapters through `Program.cs`.
...
- [ ] Allow only explicit adapter configuration; mask tokens in settings/test
...
- [ ] Commit: `feat(prefetch): warm upcoming imported episodes from verified playback`.

## Task 9 — Complete optional smart policies and cache administration

**Create under `backend/Services/Prefetch/`:** `HistoryPrefetchPolicy.cs`,
`CollectionPrefetchPolicy.cs`, `MinimumRangePrefetchPolicy.cs`.
**Create under `backend/Services/NativeCache/`:** `NativeCacheRootPolicy.cs` only
when enabling multiple roots; retain the single-root store protocol.
**Modify:** corresponding config/UI, existing Watchtower event boundary if used;
new `Services/Prefetch/PredictionPolicyTests.cs`, `Services/NativeCache/RootPolicyTests.cs`.
...
- [ ] Feed resolved imported IDs from existing Watchtower/list sources into the
  same queue. Do not duplicate release resolution, indexer queries or Arr searches.
...
- [ ] Add optional multiple roots only after single-root canary: fixed placement
  per generation, per-root budgets/owner locks, read-only roots, bounded availability
  probes, and failover to uncached source. Never silently copy between roots.
- [ ] Validate folder removal while active, unavailable volumes and recovery.
  Network roots remain disabled unless tested for locking/rename/durability.
- [ ] Commit policies and root support separately with scoped `feat(prefetch)`
  and `feat(cache)` messages. The complete feature claim requires this phase;
  the earlier cache/manual canary is explicitly partial coverage.
...
- [ ] Run automated matrix in PR CI: direct/multipart/encrypted, both cache flags,
...
- [ ] Compare four storage modes: existing article only, native only, native with
  article hits/admission suppression, and existing rclone baseline on a disposable
  mount. Quantify remaining duplication and rclone interaction explicitly.
...
5. Enable native manual cache first, read-through next, then verified-next-episode,
   then history/collections. Collect per-state queue samples and committed coverage;
...
| B: integrity and native read-through | 2–5 after A | Verified partial/full cache with safe eviction | 1–2 weeks |
| C: manual warm/UI canary | 6–7 after B | Testable cache/queue slice with flags off by default | 3–5 days |
| D: smart policy parity | 8–9 after C | Optional verified/history/collection producers | 1–2 weeks |
