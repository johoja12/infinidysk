...
public sealed class NzbDavCanaryLinkPlanner
{
    ...
    public Task<NzbDavCanaryPlanResult> GenerateAsync(
        ...
        string packageRoot,
        long runId,
        IReadOnlySet<Guid>? unlinkedIds = null,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var selectedIds = package.Manifest.SelectedLinks
            ...
            .ToArray();
        if (unlinkedIds is not null
            && unlinkedIds.Any(id => !selectedIds.Contains(id.ToString(), StringComparer.OrdinalIgnoreCase)))
            throw new InvalidDataException("An unlinked ID is not selected by this package.");
        ...
        var plan = new NzbDavCanaryPlan(
            ...
            links.Count,
            actionable,
            links.Count + (unlinkedIds?.Count ?? 0) == package.Manifest.SelectedLinks.Count
