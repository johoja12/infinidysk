...
const file = { key: "a".repeat(64), folderId: "disk", itemId: "item", name: "Film.mkv",
  length: 100, allocatedBytes: 55, verifiedBytes: 50, pinned: false,
  lastAccessedAt: "2026-09-23T00:00:00Z", accessCount: null };
const summary = { available: true, activeMode: "native", configuredMode: "native", restartRequired: false,
  initializationPending: false, initializationError: null, asOfUtc: "2026-09-23T00:00:00Z",
  totalAllocatedBytes: 55, totalQuotaBytes: 100, liveFiles: 1, emptyEntries: 0,
  verifiedBlocks: 1, retiredEntries: 0, retiredBytes: 0,
  traffic: { hitBlocks: 2, hitBytes: 100, missBlocks: 1, missBytes: 50, committedBytes: 50 },
  folders: [{ id: "disk", name: "NAS", path: "/cache", storageType: "nas", priority: 1,
    enabled: true, readOnly: false, maxBytes: 100, minFreeBytes: 0, online: true,
    writable: true, error: null, allocatedBytes: 55, liveFiles: 1, emptyEntries: 0,
    verifiedBlocks: 1, retiredEntries: 0, retiredBytes: 0 }] };
...
function setupFetch() {
  const fetchMock = vi.fn((input: string, _init?: RequestInit) => {
    ...
    if (input.includes("/files")) return Promise.resolve(json({ items: [file], totalCount: 1, nextCursor: null }));
    if (input.includes("/ranges")) return Promise.resolve(json({ ranges: [{ offset: 0, count: 50 }], nextAfter: null }));
    if (input.includes("/evictions")) return Promise.resolve(json({ items: [], totalCount: 0, nextCursor: null }));
...
describe("Native Cache browser", () => {
  it("shows catalogue metrics and sparse file details without issuing a write request", async () => {
    ...
    render(<MemoryRouter><NativeCachePage /></MemoryRouter>);
    ...
    await waitFor(() => expect(fetchMock.mock.calls.some(call => call[0].includes("/ranges"))).toBe(true));
  ...
  it("filters eviction history by reason", async () => {
    ...
    await waitFor(() => expect(fetchMock.mock.calls.some(call => call[0].includes("reason=pressure"))).toBe(true));
  });
});
