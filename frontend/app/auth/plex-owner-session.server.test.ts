...
it("rejects read-only and tampered cookies and upgrades a signed legacy admin cookie", async () => {
  ...
  vi.stubEnv("DISABLE_FRONTEND_AUTH", "false");
  const auth = await import("./authentication.server");
  const readonly = await auth.setSessionUser(new Request("http://localhost/"), "reader", "readonly");
  const request = (cookie: string) => new Request("http://localhost/api/plex/accounts", { headers: { Cookie: cookie.split(";", 1)[0]! } });
  expect(await auth.ensurePlexOwnerSession(request(readonly.headers["Set-Cookie"]))).toEqual({ owner: null });
  expect(await auth.ensurePlexOwnerSession(request("__session=forged-admin"))).toEqual({ owner: null });
  const { createCookieSessionStorage } = await import("react-router");
  const legacy = createCookieSessionStorage({ cookie: { name: "__session", secrets: ["plex-owner-test"], path: "/" } });
